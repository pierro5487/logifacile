...
class FacturePdf extends Fpdf {
	...
	public function setAutoE($autoE){
		$this->AE = $autoE;
	}
	...
	public function totauxFooter($totaux) {
		...
		$this->SetX($xPosRight);
		// partie reglement
		if (count($totaux['encaissements']) != 0) {
			$this->SetFont('Arial', 'B', $this->font - 1);
			$this->SetColor();
			...
			foreach ($totaux['encaissements'] as $encaissement) {
				$this->Cell(25, $this->cellHeight, $encaissement['date']->format('d/m/Y'), 'B', 0, 'C');
				$this->Cell(40, $this->cellHeight, strtoupper($encaissement['mode_reglement']), 'B', 0, 'C');
				$this->Cell(25, $this->cellHeight, number_format($encaissement['montant'], 2, ',', ' ') . ' ' . chr(128), 'B', 1, 'C');
			}
			$this->ln(10);
		}
		//partie tva
		...
		$this->SetColor();
		$this->SetFont('Arial', 'B', $this->font);
		$this->Cell(60, $this->cellHeight, 'Montant HT', 0, 0, 'R');
		$this->SetColor('black');
		...
		if(!$this->AE) {
			...
			$this->SetColor();
			$this->SetFont('Arial', 'B', $this->font + 2);
			$this->Cell(60, $this->cellHeight, 'Total TTC', 0, 0, 'R');
			$this->SetColor('black');
			$this->Cell(30, $this->cellHeight, number_format($totaux['totalTTC'], 2, ',', ' ') . chr(128), 0, 1, 'R');
			$this->SetX($xPosRight);
		}
		$this->SetColor();
		$this->SetFont('Arial', '', $this->font);
		...
		$this->SetColor();
		$this->SetFont('Arial', 'B', $this->font+2);
		$this->Cell(60,$this->cellHeight,utf8_decode('Net à payer'),0,0,'R');
		$this->SetColor('black');
		$this->Cell(30,$this->cellHeight,number_format($totaux['netAPaye'],2,',',' ').chr(128),0,1,'R');
		if($this->AE){
			$this->ln();
			$this->SetColor();
			$this->SetFont('Arial', '', $this->font);
			$this->Cell(0,$this->cellHeight,'TVA non applicable art. 293 b du cgi',0,1,'R');
		}
	}
...
//		$this->SetFont('','U');
		$this->SetX(55);
		$this->Write(5,utf8_decode('Auto-entreprise enregistré le 17-06-2009 | SIRET : 	512 946 591 00017'));
		$this->ln();
		$this->SetX(65);
		$this->Write(5,utf8_decode('Centre montage agrée | dépannage | fourriere agréé'));
		//numero de page
		$this->SetFont('','');
		$this->SetTextColor(0,0,0);
		$this->SetY(-10);
